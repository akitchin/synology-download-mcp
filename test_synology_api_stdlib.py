...
class SynologyDownloadStation:
    ...
    def login(self):
        ...
        # For DSM 7+, auth is typically handled through entry.cgi
        # Let's try with both paths
        params = {
            'api': 'SYNO.API.Auth',
            'version': '3',  # Try version 3 first for newer DSM
            'method': 'login',
        ...
        if data.get('success'):
            ...
            if isinstance(error_value, dict):
                # If error is a dict, try to get the code from it
                error_code = error_value.get('code', 0)
                error_msg = error_codes.get(error_code, f"Error details: {json.dumps(error_value)}")
            else:
